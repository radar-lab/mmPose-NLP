...
DataUARTHandler::DataUARTHandler(ros::NodeHandle* nh) : currentBufp(&pingPongBuffers[0]) , nextBufp(&pingPongBuffers[1]) {
    //DataUARTHandler_pub = nh->advertise<sensor_msgs::PointCloud2>("/multiradar_mmpose/radar_scan_pcl", 100);
    radar_scan_pub = nh->advertise<multiradar_mmpose::RadarScan>("/multiradar_mmpose/radar_scan", 100);
    marker_pub = nh->advertise<visualization_msgs::Marker>("/multiradar_mmpose/radar_scan_markers", 100);
    ...
    // Wait for parameters
    while(!nh->hasParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/doppler_vel_resolution")){}

    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/numAdcSamples", nr);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/numLoops", nd);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/num_TX", ntx);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/f_s", fs);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/f_c", fc);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/fc_chirp", fc_chirp);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/BW", BW);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/PRI", PRI);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/t_fr", tfr);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/max_range", max_range);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/range_resolution", vrange);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/max_doppler_vel", max_vel);
    nh->getParam("/multiradar_mmpose/radar_" + myNodeID.substr(myNodeID.length()-1) + "/doppler_vel_resolution", vvel);
...
void *DataUARTHandler::sortIncomingData( void )
{
    ...
    multiradar_mmpose::RadarScan *radarscan_cf = NULL; // current frame
    multiradar_mmpose::RadarScan radarscan_pub; // last frame
    std::string myNodeID = ros::this_node::getName();
	std::string frame_id = "radar_" + myNodeID.substr(myNodeID.length()-1);
    ...
    while(ros::ok())
    {
        ...
        case READ_POINT_CLOUD_2D:
            ...
            if(radarscan_cf == NULL)
            {
                radarscan_cf = (multiradar_mmpose::RadarScan * )malloc(num_Points * sizeof(multiradar_mmpose::RadarScan));
            }
            //num_Points = (tlvLen-sizeof(mmwData.tlvHeader))/sizeof(mmwData.PointCloudData);
            for(i=0; i<num_Points; i++)
            {
                // Refer to C:\ti\mmwave_industrial_toolbox_4_0_0\labs\multiradar_mmpose\18xx_68xx_multiradar_mmpose\gui\getGtrackPtCloud.m
                //get range
        ...
        case READ_TARGET_INDEX: 
            ...
            if(Point_List_Idx == 4){
                ...
                for(i=0; i<num_TargetIdx; i++)
                {
                    radarscan_pub.header.frame_id       = frame_id;
...
void DataUARTHandler::visualize(const multiradar_mmpose::RadarScan &msg){
    std::string myNodeID = ros::this_node::getName();
    std::string frame_id = "radar_" + myNodeID.substr(myNodeID.length()-1);
